...
def classView(request, class_code):
    ...
    #exams = Exam.objects.filter(classes=classes.id)
...
def examDetailsView(request, exam_code):
    ...
    if request.method == "POST":
        if request.POST.get('action') == "updateexam":
            Exam.objects.filter(id=exam_code).update(name=request.POST.get('exam_name'), duration= int(request.POST.get('exam_duration')), instruction=request.POST.get('exam_instruction'), date=datetime.strptime(request.POST.get('exam_date'),'%Y-%m-%d'), start_time=datetime.strptime(request.POST.get('exam_start_time'), '%H:%M'), end_time=datetime.strptime(request.POST.get('exam_end_time'), '%H:%M'))            
...
def examView(request, exam_id):
    ...
    for question in question_obj:
        if question.id in done_questions:
            ...
            data = {"question": question, 'answersheet': answersheet[0].id,'total': question_obj.count(), 'done': done_answers.count()+1}
            break
